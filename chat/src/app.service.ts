...
export class AppService {
  ...
  getFormatedIntents(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let a = 0; a < data[i].responses.length; a++) {
        iterationResponses.push(data[i].responses[a].response);
      }
  ...
  classifyUserInput(userInput: string, language: string) {
    ...
    return {
      predicted_answer: randomResponse || 'Sorry, I didn\'t understand that. Could you please rephrase?',
      intent,
      tokens,
    }
